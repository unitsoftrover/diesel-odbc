...
/// Statements
pub enum ResultSetState<AC: AutocommitMode> {
    Data(Statement<AC>),
    NoData(Statement<AC>),
}
...
pub struct Statement<AC: AutocommitMode> {
...
pub struct Cursor<'s, AC: AutocommitMode> {
    stmt: &'s mut Statement<AC>,
    buffer: Vec<u8>,
}
...
impl<AC: AutocommitMode> Handle for Statement<AC> {
    type To = ffi::Stmt;
...
impl<AC: AutocommitMode> Statement<AC> {
    ...
    pub fn affected_row_count(&self) -> Return<ffi::SQLLEN> {
       self.raii.affected_row_count()
    }
    ...
    where
        ...
        T: ?Sized,
    {
        self.raii.bind_input_parameter(parameter_index, value, str_len_or_ind_ptr, enc_value)
    }
    ...
    pub fn metadata(&self) -> QueryResult<StatementMetadata> {
        ...
        let mut vec = Vec::new();       
        for i in 1..=self.num_result_cols().unwrap(){
            vec.push(self.describe_col(i as u16).unwrap());
        }
...
impl<AC: AutocommitMode> Statement<AC> {
    ...
    pub fn tables(self, catalog_name: &String, schema_name: &String, table_name: &String, table_type: &String) -> Result<Statement<AC>> {
        self.tables_str(catalog_name.as_str(), schema_name.as_str(), table_name.as_str(), table_type.as_str())
    }

    pub fn tables_str(self, catalog_name: &str, schema_name: &str, table_name: &str, table_type: &str) -> Result<Statement<AC>> {
        self.tables_opt_str(Option::Some(catalog_name), Option::Some(schema_name), Option::Some(table_name), table_type)
    }

    pub fn tables_opt_str(mut self, catalog_name: Option<&str>, schema_name: Option<&str>, table_name:Option<&str>, table_type: &str) -> Result<Statement<AC>> {
    ...
    pub fn exec_direct(mut self, statement_text: &str) -> Result<ResultSetState<AC>> {
    ...
    ///
    /// `SQLExecDirect` is the fastest way to submit an SQL statement for one-time execution.
    pub fn exec_direct_bytes(mut self, bytes: &[u8]) -> Result<ResultSetState<AC>> {
    ...
    /// Fetches the next rowset of data from the result set and returns data for all bound columns.
    pub fn fetch<'s>(&'s mut self) -> Result<Option<Cursor<'s, AC>>> {
    ...
    /// # };
    /// ```
    pub fn close_cursor(mut self) -> Result<Statement<AC>> {
...
impl<'s, AC: AutocommitMode> Cursor<'s, AC> {
...
unsafe impl<AC: AutocommitMode> safe::Handle for Statement<AC> {
...
pub struct StatementUse<'b> {
    statement: &'b mut Statement<AutocommitOn>,
    output_binds: &'b mut Binds,
    metadata: &'b StatementMetadata,
}

impl<'b> StatementUse<'b> {

    pub fn new(statement: &'b mut Statement<AutocommitOn>, output_binds: &'b mut Binds, metadata:&'b StatementMetadata) -> Self {
        StatementUse {
            statement,
            output_binds,
            metadata,
...
impl<'b> Drop for StatementUse<'b> {
    fn drop(&mut self) {
